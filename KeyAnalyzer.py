...
class KeyAnalyzer(QtCore.QObject):
    ...
    def detect_contours(self, origin: QtCore.QPoint, end_point: QtCore.QPoint):
        ...
        # Filter artifacts
        mask_white_keys = utils.get_median_filtered_mask([cv2.contourArea(c) for c in contours_white])
        ...
        # Filter artifacts
        mask_black_keys = utils.get_median_filtered_mask([cv2.contourArea(c) for c in contours_black])
        ...
        try:
            ...
            self.start_note = utils.find_start_note([c[0] for c in contours])
